...
if ($resourceorders== NULL){
	...
	$urlapi10="https://api.mercadolibre.com/shipments/$shipment/items?access_token=$mltoken";
//PUXA RESOURCE, AQUI ORDERS, DAS NOTIFICACOES

$curl10 = curl_init();

curl_setopt_array($curl10, array(
  CURLOPT_URL => $urlapi10,
...
$err10 = curl_error($curl10);

curl_close($curl10);
	
if ($err10) {
  echo "cURL Error #:" . $err10;
...
$mlresponde10 = json_decode($response10,true);
//$xxx=$mlresponde10[0]['order_id'];
//echo "<br><br>";
//echo $response10;
//echo "<br><br>";
//echo $urlapi10;
...
if (!isset($mlresponde10[0]['order_id'])){
	die ("<br><font color='red' face='verdana'><b>ERRO: Problema no order_id</b></font><br><font color='black' face='verdana'><b>Exclua os dados para continuar.</b></font>");
	(header("Refresh: 120;"));
}else{
$orderidnops=$mlresponde10[0]['order_id'];
$resourceorders="/orders/$orderidnops";
//echo "$resourceorders";
//die("<br>TESTE");
}
}	
...
$notaSKU="{'note':'Envio sem pedido ou pedido sem SKU. Tentar novamente ou lançar manualmente'}";
...
$ch = curl_init('USERHOSTURL:8867/root/cliente');
...
$nota="{'note':'Pedido Lançado no sistema. $numerovenda'}";
$urlnotes="https://api.mercadolibre.com$resourceorders/notes?access_token=$mltoken";
$ch = curl_init($urlnotes);
curl_setopt_array($ch,[
    CURLOPT_CUSTOMREQUEST=>"POST",
...
$ch4 = curl_init('USERHOSTURL:8867/root/venda');
...
$notaG="{'note':'Pedido Lançado no sistema. #$numerovenda4'}";
...
$ch5 = curl_init('USERHOSTURL:8867/root/cliente');
...
$ch6 = curl_init('USERHOSTURL:8867/root/venda');
